...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  ...
  if (!response.text) {
    throw new Error('No fue posible converter el audio')
  }
...
export async function generateEmbeddings(text: string) {
  ...
  if (!response.embeddings?.[0].values) {
    throw new Error('No fue posible generar los embeddings.')
  }
...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    INSTRUÇÕES: 
      ...
      - Se a resposta não for encontrada no contexto, apenas responda que não possui informações suficientes para responder tal pergunta";
      - Seja objetivo;
